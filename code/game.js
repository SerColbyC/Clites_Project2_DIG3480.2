...
var actorChars = {
  ...
  "=": Lava, "|": Lava, "v": Lava,
  "c": Gem //new, should effect player
  //Coin, Gem, stopped wobbeling, and player stopped moving when I updated code
};
...
function Level(plan) {
  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      else if (ch =="y") //new - dark grey background element
        fieldType = "floater";

      else if (ch =="p") //portal which respawns player - this is the hazard
        fieldType = "portal";
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin" || type == "gem") { //use an OR statement to collect gem, now make gem do something
  ...
  else if (type == "portal") //new - portal
  {
    this.player.pos = new Vector(10,10);
  }

  if (type == "gem") //gem action
  {
    playerXSpeed = 30;
  }
